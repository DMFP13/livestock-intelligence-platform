    ...
    if platform_service is None:
            ...
            if not operator_rows:
                ...
                with c2:
                    if st.button("Test connector config", key="op_test_cfg"):
                        st.json(run_connector_config_test(platform_service, selected_id))
                with c3:
        ...
        if live_source_health and live_source_health.get("sources"):
            st.caption(
                ...
                f" (rolling window: {live_source_health.get('rolling_window_days', 7)}d)"
            )
            view_rows = []
            for row in live_source_health.get("sources", []):
                view_rows.append(
                    {
                        "connector": row.get("connector_key"),
                        "source_system": row.get("source_system"),
                        "mode": row.get("mode"),
                        "active": "yes" if int(row.get("is_active") or 0) == 1 else "no",
                        "health": row.get("health_class", "unknown"),
                        "latest_run_status": row.get("latest_run_status") or row.get("status"),
                        "recent_runs": row.get("recent_runs", 0),
                        "recent_failures": row.get("recent_failed_runs", 0),
                        "recent_failure_rate": row.get("recent_failure_rate", 0.0),
                        "recent_validation_errors": row.get("recent_validation_errors", 0),

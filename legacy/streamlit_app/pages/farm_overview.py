...
def render_farm_overview(
    ...
    if not farm_ts.empty:
        ...
        if trend_cols:
            melt = farm_ts[["date", *trend_cols]].melt(id_vars="date", var_name="series", value_name="value")
            chart = (
        ...
        if state_cols:
            melt = farm_ts[["date", *state_cols]].melt(id_vars="date", var_name="series", value_name="value")
            chart = (
                alt.Chart(melt.dropna(subset=["value"]))
                .mark_line(point=False)
                .encode(
